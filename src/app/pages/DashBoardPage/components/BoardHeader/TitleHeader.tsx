...
import {EditOutlined, LeftOutlined, MoreOutlined, SendOutlined} from '@ant-design/icons';
...
export const TitleHeader: FC = memo(() => {
  ...
  const toBoardEditor = () => {
    ...
    if (pathName.includes(boardId)) {
      ...
      history.push(
        `${urls.editBoardPageUrl}`
      );
    }
  };
  const handleCloseBoardEditor = () => {
    // if(!onCloseBoard){
    //   history.goBack();
    // }else{
    //   onCloseBoard();
  ...
  return (
    <Wrapper>
      <div className={classnames('backBtn',{ disabled: status < 2 })}>
        <LeftOutlined onClick={handleCloseBoardEditor} />
        {/*{title}*/}
      </div>
