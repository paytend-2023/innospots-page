...
export const ChartDraggableTargetContainer: FC<ChartDataConfigSectionProps> =
  memo(function ChartDraggableTargetContainer({
    ...
    if (type == 'field' && Array('group', 'aggregate','mixed').indexOf(config.type || '') != -1) {
      return (
        <StyledBaseDataConfigFieldSection>
          <div className={"baseDataConfigFieldSection"}>
            <div className={"baseDataConfigFieldTitle"}>
              {t(config.label || '') }
              {extra?.()}
            </div>
    ...
    }else if(type=='setting' && Array('group','aggregate','mixed').indexOf(config.type||'')==-1){
      return (
        <StyledBaseDataConfigSection>
          <div className={"baseDataConfigSection"}>
            <div className={"baseDataConfigSectionTitle"}>
              {t(config.label || '') }
              {extra?.()}
            </div>
            <StyledContainer ref={drop} isOver={isOver} canDrop={canDrop}>
