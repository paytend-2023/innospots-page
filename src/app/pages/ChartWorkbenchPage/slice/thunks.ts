...
export const initWorkbenchAction = createAsyncThunk(
  ...
  async (
    ...
    try {
        await thunkAPI.dispatch(fetchDataViewsAction(reqInfo));
      if (reqInfo.arg.backendChartId) {
        await thunkAPI.dispatch(
          workbenchSlice.actions.saveBackendChartId(reqInfo.arg.backendChartId),
        );
        await thunkAPI.dispatch(
          fetchChartAction({ chartId: reqInfo.arg.backendChartId }),
        );
        await thunkAPI.dispatch(refreshDatasetAction({}));
      } else if (reqInfo.arg.backendChart) {
        await thunkAPI.dispatch(
          fetchChartAction({ backendChart: reqInfo.arg.backendChart }),
        );
        await thunkAPI.dispatch(refreshDatasetAction({}));
      }
    } catch (error) {
...
export const fetchAvailableSourceFunctionsForChart = createAsyncThunk<
  string[],
  string
>('workbench/fetchAvailableSourceFunctionsForChart', async viewId => {
  try {
    const data = await fetchAvailableSourceFunctionsAsync(viewId);
    return data;
  } catch (err) {
    throw err;
  }
